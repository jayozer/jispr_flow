...
- It never silently clobbers an existing config: overwriting requires an
  explicit "y", and even then it merges -- keys the wizard did not ask about
  are read from the existing file and preserved.
"""
...
def _toml_value(value: object) -> str:
    """Render one TOML value: the wizard's own answers are strings, but keys
    preserved from an existing config (see ``run_wizard``'s overwrite merge)
    keep whatever type ``tomllib`` parsed -- an int ``vad_silence_ms`` must
    not come back quoted.
    """
...
def _render_toml(values: Mapping[str, object]) -> str:
...
def run_wizard(
    ...
    Writes the keys the wizard asked about, to a temp file that is validated
    with :func:`local_flow.config.load_config` before being swapped into
    place; on validation failure the temp file is discarded (and the error
    re-raised) without touching any pre-existing config at the target path.
    When overwriting an existing config, its keys are read first and every
    key the wizard did not ask about is preserved (a relocated ``data_dir``,
    tuned VAD thresholds, ...) -- overwrite means merge, not replace.
    """
    ...
    # Overwrite means merge, not replace: every key the wizard did not ask
    # about (data_dir, VAD tuning, ...) is carried over from the existing
    # config, and the wizard's answers win for the keys it did ask.
    ...
    if write_path.exists():
        ...
        if answer.strip().lower() not in ("y", "yes"):
            ...
            return write_path
        try:
            existing = tomllib.loads(write_path.read_text(encoding="utf-8"))
        except (OSError, tomllib.TOMLDecodeError) as exc:
            say(
                f"warning: could not parse the existing config ({exc}); "
                "its other settings cannot be preserved -- writing only the "
                "wizard's answers."
            )
